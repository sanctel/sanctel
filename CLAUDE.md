...
- **tmux session/window mapping** —
  ...
  One tmux session per Tab (`sanctel_wt_<worktreeId>__<windowName>`),
  one window per session, Worktree as name prefix. Sanctel runs on a
  dedicated tmux server (`-L sanctel`).
